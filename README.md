...
## 概要
- 表形式のテキストデータファイル、特にTSV形式のファイルを扱うためのコマンド群およびその為に発生する周辺的な操作を行うためのコマンド群である。
TSVとは[Tab-Separated Values](https://www.iana.org/assignments/media-types/text/tab-separated-values)である。<!--なお、このモジュールをインストールすると、同じ作者が作成した、他のCPANモジュール群をインスト
